...
async function fetchStockData(ticker) {
  ...
  const response = await fetch(
    `/api/stock/${ticker}?period=${period}&interval=${interval}`
  );
  if (!response.ok) {
...
async function fetchSignals(ticker) {
  ...
  const response = await fetch(
    `/api/signals/${ticker}?period=${period}&interval=${interval}`
  );
  if (!response.ok) {
    const error = await response.json();
...
async function fetchBacktest(ticker) {
  ...
  const response = await fetch(
    `/api/backtest/${ticker}?period=${period}&interval=${interval}`
  );
  if (!response.ok) {
    const error = await response.json();
...
async function fetchPredictions(ticker) {
  ...
  const response = await fetch(
    `/api/predict/${ticker}?period=${period}&interval=${interval}`
  );
  if (!response.ok) {
    const error = await response.json();
...
function updatePredictionChart(predictions, stockData) {
  ...
  const actualPrices = [
    ...last30Prices,
    ...Array(predictions.predictions.length).fill(null),
  ];
  const predictedPrices = [
    ...Array(last30Days.length).fill(null),
                        ...
                        <div class="news-item">
                          ...
                          article.title
                            ...
                            <div class="news-meta">
                                ...
                                <span class="news-sentiment" style="color: ${
...
function updateMetrics(backtest) {
  ...
  profitEl.textContent = `$${backtest.total_profit.toLocaleString()}`;
  profitEl.style.color = backtest.total_profit >= 0 ? "#10b981" : "#ef4444";

  // 2. ROI (New)
  const roiEl = document.getElementById("roi"); // Make sure ID matches HTML
  if (roiEl) {
    roiEl.textContent = `${backtest.roi}%`;
    roiEl.style.color = backtest.roi >= 0 ? "#10b981" : "#ef4444";
  }

  // 3. Portfolio Value (New)
  const portEl = document.getElementById("portfolioValue"); // Make sure ID matches HTML
  if (portEl) {
    portEl.textContent = `$${backtest.final_value.toLocaleString()}`;
  }
